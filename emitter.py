...
def emit_cpp_header(
    ...
    if relative_degree == 2:
        # Warn (in the header) if the network uses ReLU.
        for layer in network.activation_layers:
            if layer.kind == "relu":
                # Not fatal; we emit a #warning. ReLU's second derivative is
                # zero almost everywhere, so the Hessian-vector contribution
                # vanishes -- accurate but probably not what the user wants.
                pass
...
def _emit_first_order(network: ParsedNetwork, namespace: str) -> str:
    ...
    out.write(dedent("""\
        ...
        // G  : input field G(x) flattened column-major (n x m)
...
def _emit_second_order(network: ParsedNetwork, namespace: str) -> str:
    ...
    # Check: warn at compile-time if relu is in the network
    has_relu = any(a.kind == "relu" for a in activations)
    if has_relu:
        out.write(
            "#warning \"Network contains ReLU activations; second-order Lie\\n"
            "         derivatives will lose the Hessian contribution because\\n"
            "         d^2/dx^2 ReLU = 0 almost everywhere. Use softplus or tanh.\"\n\n"
        )
